...
// Construct the mmproduct_data array
$box_qty = get_post_meta($product->get_id(), '_mm_box_quantity', true);
if (!$box_qty) {
    $box_qty = 12; // Default value if not set
}

// Construct the mmproduct_data array
$selected_items = !empty($selectedItems) ? $selectedItems : [];
$items = [];
foreach ($selected_items as $item_id) {
    $item_product = wc_get_product($item_id);
    if ($item_product) {
        $items[$item_id] = ["price" => $item_product->get_price()];
    }
}
$mmproduct_data = [
    "boxqty" => $box_qty,
    "items" => $items,
    "currCurrency" => get_woocommerce_currency_symbol()
];

// Encode the data as a JSON string
$mmproduct_data_json = json_encode($mmproduct_data);
?>

<!-- Hidden input field to store mmproduct_data JSON -->
<!-- Hidden input field to store mmproduct_data JSON -->
<input type="hidden" id="mmproduct_data" value='<?php echo esc_attr($mmproduct_data_json); ?>'>

<!-- Box contents display -->
<div class="hidden" id="box_contents_display">
    <h3>Current Box Contents:</h3>
    <ul id="box_items_list">
...
<script>
    ...
    jQuery(document).ready(function($) {
        // Function to update box contents display
        function updateBoxContentsDisplay() {
            var mm_pdata = getMMProductData();
            if (!mm_pdata) return;

            var boxItemsList = $('#box_items_list');
            boxItemsList.empty();

            $(".gt_box_list .gift_block").each(function() {
                if ($(this).find('img').length) {
                    var prod_id = $(this).find('img').data('id');
                    var prod_price = mm_pdata.items[prod_id].price;
                    var prod_image = $(this).find('img').attr('src');
                    var prod_name = $(this).find('img').attr('alt');

                    var listItem = '<li>';
                    listItem += '<img src="' + prod_image + '" alt="' + prod_name + '" style="width: 50px; height: 50px; border-radius: 50%; margin-right: 10px;">';
                    listItem += '<span>' + prod_name + ' - ' + mm_pdata.currCurrency + ' ' + prod_price + '</span>';
                    listItem += '</li>';

                    boxItemsList.append(listItem);
                }
            });
        }

        // Update the display initially and on each add/remove action
        updateBoxContentsDisplay();
        $('body').on("click", ".extendonsremovefilledboxes, .add_cta", function() {
            setTimeout(updateBoxContentsDisplay, 100);
        });
        ...
        // Retrieve and parse mmproduct_data safely
        function getMMProductData() {
            var mm_pdata_elem = $('#mmproduct_data');
            if (mm_pdata_elem.length === 0) {
                console.error("Element #mmproduct_data not found in the DOM");
                return null;
            }

            var mm_pdata = mm_pdata_elem.val();
            console.log("mmproduct_data raw value:", mm_pdata); // Debugging line
            if (!mm_pdata) {
                console.error("mmproduct_data is undefined or null");
                return null;
            }

            mm_pdata = safeParseJSON(mm_pdata);
            if (!mm_pdata) {
                console.error("Failed to parse mmproduct_data");
                return null;
            }

            console.log("Parsed mmproduct_data:", mm_pdata); // Debugging line
            return mm_pdata;
        }

        // Remove class and attribute on product removed
        $('body').on("click", ".extendonsremovefilledboxes", function(e) {
            e.preventDefault();
            var mm_pdata = getMMProductData();
            if (!mm_pdata) return;

            var targ = $(this).closest('.gift_block'); // Adjusted selector to target the closest .gift_block
            var prod_id = targ.find('img').data('id');
            targ.find('img').remove();
            targ.removeClass('mm_yes prefilleditem').removeAttr('data-pid');

            // Update hidden field
            updateProductItems(mm_pdata.boxqty);

            // Hide 'Add New Box' button
            $('#add_new_box').hide();

            // Decrease price if necessary
            mm_price_decrease(mm_pdata, prod_id);

            // Update box contents display
            updateBoxContentsDisplay();
        });

        // Adding products to cart
        $('body').on('click', '.add_cta', function(e) {
            e.preventDefault();
            var mm_pdata = getMMProductData();
            if (!mm_pdata) return;

            var targ = $(this);
            var prod_id = targ.data('id');
            var qty = $('#product-quantity' + prod_id).val();

            for (var i = 0; i < qty; i++) {
                if (mm_count_product_number() >= mm_pdata.boxqty) {
                    mm_stack_error();
                    return;
                }
                addProductToBox(mm_pdata, prod_id, mm_pdata.boxqty);
            }
        ...
        function addProductToBox(mm_pdata, prod_id, box_qty) {
            var image = $('#mm_product_image' + prod_id).attr('src');
            var box_no = $('.mm_filled_col .box-tobe-filled span:empty:first').parent().attr('id');
            $('.mm_filled_col .box-tobe-filled span:empty:first').append('<img src="' + image + '">').parent().addClass('mm_yes').attr('data-pid', prod_id);

            // Update hidden field
            updateProductItems(box_qty);

            // Increase price by product
            mm_price_increase(mm_pdata, prod_id);
        }

        function updateProductItems(box_qty) {
            var arrs = [];
            $(".box-tobe-filled").each(function() {
                if ($(this).attr('data-pid')) {
                    var prod_idd = $(this).attr('data-pid');
                    arrs.push(prod_idd);
                }
            });
            $('#mm_product_items').val(arrs.join(","));
            var final_array = arrayTo2DArray1(arrs, box_qty);
            $('#mm_product_items').val(JSON.stringify(final_array));

            // Enable/disable add to cart button
            if (enable_disabled_add_to_Cart() === 1) {
        ...
        $('#clearAllItemsBtn').on('click', function(e) {
            e.preventDefault();
            $('.box-tobe-filled').each(function() {
                $(this).find('img').remove();
                $(this).removeClass('mm_yes prefilleditem').removeAttr('data-pid');
            });

            // Update hidden field
            updateProductItems(0);
        ...
        function mm_price_decrease(mm_pdata, prod_id) {
            if (mm_pdata.box_pricing === 'perwithbase' || mm_pdata.box_pricing === 'perwoutbase') {
                ...
                if (mm_pdata.items[prod_id] && mm_pdata.items[prod_id].price) {
                    ...
                    $('.parentPrice').html(mm_pdata.currCurrency + ' ' + current);
                    $('#mmperItemPrice').val(current);
                }
            }
        }

        function mm_price_increase(mm_pdata, prod_id) {
            if (mm_pdata.box_pricing === 'perwithbase' || mm_pdata.box_pricing === 'perwoutbase') {
                var current = 0;
                var price = 0;
                current = $('.parentPrice').text();
        ...
        function mm_count_product_number() {
            var arr = [];
            if ($('#mm_product_items').val() != '') {
                arr = $('#mm_product_items').val().split(",");
            }
            return parseInt(arr.length);
        }

        function mm_stack_error() {
            $('#mm_error').show();
            setTimeout(function() {
                $('#mm_error').fadeOut(1000);
            }, 1000);
        }

        function enable_disabled_add_to_Cart() {
            var mm_pdata = getMMProductData();
            if (!mm_pdata) return 0;
