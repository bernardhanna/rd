...
<div class="px-4 md:px-0 cart_totals w-full lg:max-w-max-503 <?php echo (WC()->customer->has_calculated_shipping()) ? 'calculated_shipping' : ''; ?>">
    ...
    <div class="shop_table shop_table_responsive bg-grey-background">
        ...
        $discount_total = WC()->cart->get_discount_total() + WC()->cart->get_discount_tax();
        $total_after_discounts = $subtotal - $discount_total;
        ?>

        <div class="flex items-center justify-between w-full bg-white border-b-2 border-solid text-black-full text-base-font font-reg420 border-black-full">
            <div class="w-1/2 py-5 pl-6 text-left bg-grey-background">
                <?php esc_html_e('Total Before Shipping', 'woocommerce'); ?>
            </div>
            <div data-title="<?php esc_attr_e('Total after Discounts', 'woocommerce'); ?>">
                <?php echo wc_price( $total_after_discounts ); ?>
            </div>
        </div>
        <div class="py-4 pt-2 bg-white">
         <span class="uppercase text-black-full text-base-font font-reg420 ">Delivery calculated at checkout or Select Free collection from one of our stores </span>
        </div>
