            ...
            <div id="available-products-container" class="max-lg:hidden w-full px-0 <?php if ($product_id != 3947) : ?>lg:w-3/5 pt-8 lg:px-8<?php endif; ?> max-lg:order-2">
                <div id="available-products" class="relative grid items-center w-full grid-cols-4 gap-3 pd_box_list h-fit">
                    ...
                    ?>
                            ...
                            $product_id = $product->get_id();
                            ?>
                            <div id="product-<?php echo $product_item->get_id(); ?>" class="relative flex flex-col items-center justify-center  hover:border-yellow-300 hover:border-dashed hover:border-2 focus-visible:border-yellow-300 focus-visible:border-dotted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-300 focus-visible:ring-offset-2 cursor-pointer py-2 lg:h-[280px] <?php echo esc_attr($category_class); ?>" data-item='<?php echo esc_attr(json_encode($product_data)); ?>'>
                                ...
                                <?php else : ?>
                                    ...
                                    <p class="pt-4 leading-none text-center pd_title text-black-full font-reg420 text-mob-md-font max-w-[190px]">
                                        <?php echo esc_html($product_name); ?>
                                    </p>
                            <p class="in-box-count text-sm text-gray-600 py-2" data-pid="<?php echo $product_item->get_id(); ?>"></p>
                          ...
                          <button
                            type="button"
                            class="font-bold plus-btn px-8 h-[40px] max-w-[110px] mobile:max-w-160px
                                py-3 my-4 btn text-black-full text-sm-font lg:text-sm-font font-normal
                                bg-yellow-primary rounded-lg-x w-auto rd-border hover:text-black-full
                                hover:bg-white add-to-box-button"
                            data-item='<?php echo esc_attr( wp_json_encode( $product_data ) ); ?>'
                            data-pid="<?php echo esc_attr( $product_item->get_id() ); ?>"
                        >
                            Add
                        </button>
                                                        </div>
...
<script>
  var boxItems = [];
    ...
    boxItems.forEach(i => updateAddButton(i.id));
    });
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".product-quantity-input").forEach((inp) => {
        ...
        // 🔁 Button logic here
        const btn = document.querySelector(`.add-to-box-button[data-pid="${pid}"]`);
        if (btn) {
        if (qty > 0) {
            btn.textContent = '+';
            btn.className =
            'font-bold minus-btn border-2 border-yellow-primary hover:border-black-full hover:bg-white bg-yellow-primary rounded-full h-[40px] w-[40px]';
        } else {
            btn.textContent = 'Add';
            btn.className =
            'font-bold plus-btn px-8 h-[40px] max-w-[110px] mobile:max-w-160px py-3 my-4 btn text-black-full text-sm-font lg:text-sm-font font-normal bg-yellow-primary rounded-lg-x w-auto rd-border hover:text-black-full hover:bg-white add-to-box-button';
        }
        }
    });
    }
...
function updateBoxDisplay() {
  if (productId === 3947) {
    // This part for product ID 3947 (custom order) remains the same
  ...
  // Iterate up to the maximum quantity and fill slots
  for (let i = 0; i < max; i++) {
    ...
    if (i < flatBoxItems.length) {
      ...
      // This slot is empty, add a placeholder
      w.classList.add("placeholder");
      w.innerHTML = `<img src="${ph}" class="box-img" alt="Donut Placeholder">`;
    }
    box.appendChild(w);
  }
  ...
  document
    .querySelectorAll('.add-to-box-button')
    .forEach(btn => updateAddButton(Number(btn.dataset.pid)));
};
...
<script>
    ...
    // 1️⃣ Patch afterMutation so it also closes mobile when full and refreshes all counters
const _origAfter = afterMutation;
afterMutation = function() {
  _origAfter && _origAfter();

  // update every “N in box” text (desktop & mobile)
  document.querySelectorAll('.in-box-count').forEach(el => {
    const pid = parseInt(el.dataset.pid, 10);
    updateInBoxCount(pid);
  });

  // if we hit max, close mobile modal
  const max = parseInt(document.getElementById('box-container').dataset.maxQuantity, 10) || 0;
...
// 3️⃣ Wire up the mobile “+” buttons
document.querySelectorAll('#available-products-mobile .plus-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    const item = JSON.parse(btn.dataset.item);
    addItemToBox(item, 1);
    toggleMinusBtn(item.id);
...
// 4️⃣ Wire up the mobile “–” buttons
document.querySelectorAll('#available-products-mobile .minus-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    const pid = parseInt(btn.dataset.pid, 10);
    removeOneItemFromBox(pid);
    toggleMinusBtn(pid);
...
<script>
    ...
    <script>
    (function(){
    // 1) Make sure this lives at the top of your <script> so both functions can see it:
    function updateAddButton(pid) {
    const qty = (boxItems.find(i => i.id === pid) || {}).quantity || 0;
    const btn = document.querySelector(`.add-to-box-button[data-pid="${pid}"]`);
    if (!btn) return;
    if (qty > 0) {
        btn.textContent = '+';
        btn.setAttribute('aria-label','Already in box');
        btn.className = 'font-bold minus-btn border-2 border-yellow-primary hover:border-black-full hover:bg-white bg-yellow-primary rounded-full h-[40px] w-[40px] add-to-box-button';
    } else {
        btn.textContent = 'Add';
        btn.setAttribute('aria-label','Add to box');
        btn.className = 'font-bold plus-btn px-8 h-[40px] max-w-[110px] mobile:max-w-160px py-3 my-4 btn text-black-full text-sm-font lg:text-sm-font font-normal bg-yellow-primary rounded-lg-x w-auto rd-border hover:text-black-full hover:bg-white add-to-box-button';
    }
    }
  ...
  /** 2️⃣ Patch afterMutation **for all buttons**, so removal toggles as well **/
    const _origAfter = window.afterMutation || function(){};
    window.afterMutation = function(){
    _origAfter();
    document
        .querySelectorAll('.add-to-box-button')
        .forEach(btn => updateAddButton(Number(btn.dataset.pid)));
  ...
  /** 3️⃣ On initial load, run once against every button **/
    document.addEventListener('DOMContentLoaded', () => {
    document
        .querySelectorAll('.add-to-box-button')
        .forEach(btn => updateAddButton(Number(btn.dataset.pid)));
    ...
    // Whenever you click “Clear Box” (desktop or mobile), re-run updateAddButton for every product
  ['clear-box','mobile-clear-box'].forEach(id => {
    const btn = document.getElementById(id);
    if (!btn) return;
    btn.addEventListener('click', () => {
      // short delay to let clearBoxItems() do its thing
      setTimeout(() => {
        document
          .querySelectorAll('.add-to-box-button')
          .forEach(el => updateAddButton(Number(el.dataset.pid)));
      }, 50);
    });
  });
})();
